    ...
    try:
        for i in range(n,len(lst)):
            ...
            if article.title not in title:
                ...
                resultBox.itemconfigure(END,fg="green")
                resultBox.insert(END,f"URL: {article.url}")
                resultBox.itemconfigure(END,fg="green")
                resultBox.insert(END,f"Title: {article.title}")
                resultBox.itemconfigure(END,fg="green")
                resultBox.insert(END,f"Time: {article.date_publish}")
                resultBox.itemconfigure(END,fg="green")
                resultBox.insert(END,f"Domain: {article.source_domain}\n")
                resultBox.itemconfigure(END,fg="green")
...
def main(keyword):
    ...
    lst = file.readlines()
    resultBox.insert(END,f"Total {len(lst)} results found for {keyword}")
    resultBox.itemconfig(END, fg = "red")
    random.shuffle(lst)
    try:
...
searchEntry = Entry(bottomFrame, width = 35, font=10, textvariable = searchKeyword, bg = "#E0E0E0", bd = 5)
...
windowResult = Tk()
lower_frame = Frame(windowResult, bg="#80c1ff")
lower_frame.pack(fill = BOTH, expand = True)

resultBox = Listbox(lower_frame,font=fot,bg="black")
...
timeLabel = Label(bottomFrame, text = f"Current Time: {time.asctime()}", font=("Scary",11,"bold"), fg = "blue")
timeLabel.place(x = 388, y = 225)
